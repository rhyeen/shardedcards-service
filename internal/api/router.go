...
// GameHandler see handlers for more details.
type GameHandler interface {
	CreateGame(w http.ResponseWriter, r *http.Request, p httprouter.Params)
}
...
func handleAuthRoutes(handler *httprouter.Router, apiPath string, routerHandlers RouterHandlers) {
	handler.POST(fmt.Sprintf("/%v/game", apiPath), routerHandlers.GameHandler.CreateGame)
}
